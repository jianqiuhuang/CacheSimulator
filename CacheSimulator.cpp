...
long long Cache_simulator::simulate(int cache_size, int cache_line_size, int associativity, std::string policy){
	...
	if(cache_type == DIRECT_MAPPED)
		...
		cache_hit = fifo_set_associative(cache_size, cache_line_size, cache_size/cache_line_size);
	else{
...
long long Cache_simulator::set_associative_nextline_prefetching(int cache_size, int cache_line_size, int associativity){
	...
	total_referenced  = 0;
		...
		for(unsigned int i = 0; i < cache.size(); i++){
                        if(cache[i][cache_index].cache_line[0] == (base*cache_line_size)){
                                cache[i][cache_index].last_used_time = std::chrono::duration<double, std::milli> (std::chrono::high_resolution_clock::now() - start_time).count();
				nextline_hit = true;
				break;
			}
		...
		if(!nextline_hit){
			...
			for(unsigned int i = 0; i < cache.size(); i++){
				if(cache[i][cache_index].last_used_time < min){
        	        ...
        	        if(DEBUG){
                		std::cerr << "next line prefetching, way " << victim_index << " at line " << cache_index << " contains ";
	                        for(unsigned int i = 0; i < cache[victim_index][cache_index].cache_line.size(); i++)
        	                	std::cerr << "0x" << std::hex << cache[victim_index][cache_index].cache_line[i] << " ";
                	                std::cerr << std::endl << std::endl;
	        	}
...
long long Cache_simulator::fifo_set_associative(int cache_size, int cache_line_size, int associativity){
	int cache_entries = cache_size / cache_line_size / associativity;
	if(DEBUG){
		std::cerr << "the cache has " << associativity << " ways and " << cache_entries << " entries in each way, and " << cache_line_size << " elements per cache line" << std::endl;
	}
